...
pdf("2001_Histogram of kTotal for high and low with cut of zero.pdf")
...
pdf("2001_Histogram of kTotal for high or low bigger then zero in one of the network.pdf")
par(mfrow=c(2,1))
...
cexLabels = 1.4
#sizeGrWindow(6,9)
pdf(file = "2001_Low_Specific-NetworkAndIGPStatistics.pdf", w=6, h=9, onefile = FALSE)
par(mfrow = c(3,2))
...
for (f in 1:2)  {
pdf(file=spaste("2001_LowHigh-LowSpecific-modulePreservation-", figNames[f],"-%02d.pdf"),
...
pdf("2001_MotivationFigure-dendrosAndTable.pdf")
fp = TRUE
